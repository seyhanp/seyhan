...
import views.html.contacts.trans_source.form;
import views.html.contacts.trans_source.index;
import views.html.contacts.trans_source.list;
...
public class TransSources extends Controller {
	...
	private final static Form<ContactTransSource> dataForm = form(ContactTransSource.class);
	 ...
	 * @return PageExtend
	 */
	private static PageExtend<ContactTransSource> buildPage() {
		List<Map<Integer, String>> dataList = new ArrayList<Map<Integer, String>>();

		List<ContactTransSource> modelList = ContactTransSource.page();
		if (modelList != null && modelList.size() > 0) {
			for (ContactTransSource model : modelList) {
		...
		return new PageExtend<ContactTransSource>(getHeaderList(), dataList, null);
	}
	...
	public static Result options(String rightName) {
		...
		Result result = ok(StringUtils.buildOptionTag(ContactTransSource.options(right), lastSaved));
		lastSaved = null;
	...
	public static Result save() {
		...
		Form<ContactTransSource> filledForm = dataForm.bindFromRequest();

		if(filledForm.hasErrors()) {
			return badRequest(form.render(filledForm));
		} else {

			ContactTransSource model = filledForm.get();
	...
	public static Result create() {
		...
		return ok(form.render(dataForm.fill(new ContactTransSource())));
	}
	...
	public static Result edit(Integer id) {
		...
		if (id == null) {
			...
			ContactTransSource model = ContactTransSource.findById(id);
			if (model == null) {
	 ...
	 * @param filledForm
	 */
	private static void checkConstraints(Form<ContactTransSource> filledForm) {
		ContactTransSource model = filledForm.get();

		if (ContactTransSource.isUsedForElse("name", model.name, model.id)) {
