...
import views.html.waybills.trans_source.form;
import views.html.waybills.trans_source.index;
import views.html.waybills.trans_source.list;
...
public class TransSources extends Controller {
	...
	private final static Right RIGHT_SCOPE = Right.IRSL_IRSALIYE_KAYNAKLARI;

	private final static Logger log = LoggerFactory.getLogger(TransSources.class);
	private final static Form<WaybillTransSource> dataForm = form(WaybillTransSource.class);
	 ...
	 * @return PageExtend
	 */
	private static PageExtend<WaybillTransSource> buildPage() {
		List<Map<Integer, String>> dataList = new ArrayList<Map<Integer, String>>();

		List<WaybillTransSource> modelList = WaybillTransSource.page();
		if (modelList != null && modelList.size() > 0) {
			for (WaybillTransSource model : modelList) {
		...
		return new PageExtend<WaybillTransSource>(getHeaderList(), dataList, null);
	}
	...
	public static Result options(String rightName) {
		...
		Result result = ok(StringUtils.buildOptionTag(WaybillTransSource.options(right), lastSaved));
		lastSaved = null;
	...
	public static Result save() {
		...
		Form<WaybillTransSource> filledForm = dataForm.bindFromRequest();

		if(filledForm.hasErrors()) {
			return badRequest(form.render(filledForm));
		} else {

			WaybillTransSource model = filledForm.get();
	...
	public static Result create() {
		...
		return ok(form.render(dataForm.fill(new WaybillTransSource())));
	}
	...
	public static Result edit(Integer id) {
		...
		if (id == null) {
			...
			WaybillTransSource model = WaybillTransSource.findById(id);
			if (model == null) {
	 ...
	 * @param filledForm
	 */
	private static void checkConstraints(Form<WaybillTransSource> filledForm) {
		WaybillTransSource model = filledForm.get();

		if (WaybillTransSource.isUsedForElse("name", model.name, model.id)) {
