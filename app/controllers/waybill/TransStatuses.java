...
import views.html.waybills.trans_status.form;
import views.html.waybills.trans_status.index;
import views.html.waybills.trans_status.list;
...
public class TransStatuses extends Controller {
	...
	private final static Form<WaybillTransStatus> dataForm = form(WaybillTransStatus.class);
	 ...
	 * @return PageExtend
	 */
	private static PageExtend<WaybillTransStatus> buildPage() {
		List<Map<Integer, String>> dataList = new ArrayList<Map<Integer, String>>();

		List<WaybillTransStatus> modelList = WaybillTransStatus.page();
		if (modelList != null && modelList.size() > 0) {
			for (WaybillTransStatus model : modelList) {
		...
		return new PageExtend<WaybillTransStatus>(getHeaderList(), dataList, null);
	}
	...
	public static Result save() {
		...
		Form<WaybillTransStatus> filledForm = dataForm.bindFromRequest();

		if(filledForm.hasErrors()) {
			return badRequest(form.render(filledForm));
		} else {

			WaybillTransStatus model = filledForm.get();
	...
	public static Result create() {
		...
		return ok(form.render(dataForm.fill(new WaybillTransStatus())));
	}
	...
	public static Result edit(Integer id) {
		...
		if (id == null) {
			...
			WaybillTransStatus model = WaybillTransStatus.findById(id);
			if (model == null) {
	...
	public static Result remove(Integer id) {
		...
		if (id == null) {
			...
			if (model == null) {
				...
				if (editingConstraintError != null) return badRequest(editingConstraintError);
				try {
					Ebean.createSqlUpdate("update waybill_trans_status set parent_id = :new_parent_id where parent_id = :parent_id")
	 ...
	 * @param filledForm
	 */
	private static void checkConstraints(Form<WaybillTransStatus> filledForm) {
		WaybillTransStatus model = filledForm.get();

		if (WaybillTransStatus.isUsedForElse("name", model.name, model.id)) {
