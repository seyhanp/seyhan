...
public class Transes extends Controller {
	 ...
	 * @return PageExtend
	 */
	private static PageExtend<OrderTrans> buildPage(OrderTransSearchParam searchParam, Right right) {
		...
		if (model.details != null && model.details.size() > 0) {
			...
			for (int i = 1; i < model.details.size() + 1; i++) {
				...
				if (std.stock.id != null && Profiles.chosen().stok_hasLimitControls) {
					...
					} else if (! isInput && stock.minLimit != null  && stock.minLimit > 0 && balance - std.netOutput < stock.minLimit) {
						veList.add(new ValidationError("stocks", Messages.get("less.than.minimum.table", i, stock.minLimit)));
					}
				}
			}
