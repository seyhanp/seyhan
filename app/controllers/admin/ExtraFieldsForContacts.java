...
import views.html.admins.extra_fields_for_contact.form;
import views.html.admins.extra_fields_for_contact.index;
import views.html.admins.extra_fields_for_contact.list;
 ...
 * @author mdpinar
*/
public class ExtraFieldsForContacts extends Controller {
	...
	private static PageExtend<AdminExtraFields> buildPage() {
		...
		List<AdminExtraFields> modelList = AdminExtraFields.page(Module.contact.name());
	...
	public static Result create() {
		...
		int rowCount = AdminExtraFields.getRowCount(Module.contact.name());
		if (rowCount > 8) {
			return badRequest(Messages.get("record.limit.alert", Messages.get("contact") +" " + Messages.get("extra_field"), 10));
		} else {
			return ok(form.render(dataForm.fill(new AdminExtraFields())));
		}
	}
	...
	public static Result edit(Integer id) {
		...
		if (id == null) {
			...
			AdminExtraFields model = AdminExtraFields.findById(Module.contact.name(), id);
			if (model == null) {
	...
	private static void checkConstraints(Form<AdminExtraFields> filledForm) {
		...
		if (AdminExtraFields.isUsedForElse("name", model.name, model.id, Module.contact.name())) {
