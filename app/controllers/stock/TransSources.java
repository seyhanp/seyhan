...
import views.html.stocks.trans_source.form;
import views.html.stocks.trans_source.index;
import views.html.stocks.trans_source.list;
...
public class TransSources extends Controller {
	...
	private final static Right RIGHT_SCOPE = Right.STOK_FIS_KAYNAKLARI;

	private final static Logger log = LoggerFactory.getLogger(TransSources.class);
	private final static Form<StockTransSource> dataForm = form(StockTransSource.class);
	 ...
	 * @return PageExtend
	 */
	private static PageExtend<StockTransSource> buildPage() {
		List<Map<Integer, String>> dataList = new ArrayList<Map<Integer, String>>();

		List<StockTransSource> modelList = StockTransSource.page();
		if (modelList != null && modelList.size() > 0) {
			for (StockTransSource model : modelList) {
		...
		return new PageExtend<StockTransSource>(getHeaderList(), dataList, null);
	}
	...
	public static Result options(String rightName) {
		...
		Result result = ok(StringUtils.buildOptionTag(StockTransSource.options(right), lastSaved));
		lastSaved = null;
	...
	public static Result save() {
		...
		Form<StockTransSource> filledForm = dataForm.bindFromRequest();

		if(filledForm.hasErrors()) {
			return badRequest(form.render(filledForm));
		} else {

			StockTransSource model = filledForm.get();
	...
	public static Result create() {
		...
		return ok(form.render(dataForm.fill(new StockTransSource())));
	}
	...
	public static Result edit(Integer id) {
		...
		if (id == null) {
			...
			StockTransSource model = StockTransSource.findById(id);
			if (model == null) {
	 ...
	 * @param filledForm
	 */
	private static void checkConstraints(Form<StockTransSource> filledForm) {
		StockTransSource model = filledForm.get();

		if (StockTransSource.isUsedForElse("name", model.name, model.id)) {
