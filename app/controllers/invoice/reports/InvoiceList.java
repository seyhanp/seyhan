...
*/
package controllers.invoice.reports;
...
import views.html.invoices.reports.invoice_list;
 ...
 * @author mdpinar
*/
public class InvoiceList extends Controller {

	private final static Right RIGHT_SCOPE = Right.FATR_FATURA_LISTESI;
	private final static String REPORT_NAME = "InvoiceList";

	private final static Form<InvoiceList.Parameter> parameterForm = form(InvoiceList.Parameter.class);
	...
	private static String getQueryString(Parameter params) {
		...
		if (params.isCash != null && params.isCash) {
			queryBuilder.append(" and t.is_cash = " + GlobalCons.TRUE);
		}
	...
	public static Result generate() {
		...
		Form<InvoiceList.Parameter> filledForm = parameterForm.bindFromRequest();

		if(filledForm.hasErrors()) {
			return badRequest(invoice_list.render(filledForm));
		} else {
	...
	public static Result index() {
		...
		return ok(invoice_list.render(parameterForm.fill(new Parameter())));
	}
