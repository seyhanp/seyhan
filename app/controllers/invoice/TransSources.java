...
import views.html.invoices.trans_source.form;
import views.html.invoices.trans_source.index;
import views.html.invoices.trans_source.list;
...
public class TransSources extends Controller {
	...
	private final static Right RIGHT_SCOPE = Right.FATR_FATURA_KAYNAKLARI;

	private final static Logger log = LoggerFactory.getLogger(TransSources.class);
	private final static Form<InvoiceTransSource> dataForm = form(InvoiceTransSource.class);
	...
	private static List<GridHeader> getHeaderList() {
		...
		headerList.add(new GridHeader(Messages.get("has_stock_effect"), "12%", true));
		headerList.add(new GridHeader(Messages.get("has_cost_effect"), "12%", true));
	 ...
	 * @return PageExtend
	 */
	private static PageExtend<InvoiceTransSource> buildPage() {
		List<Map<Integer, String>> dataList = new ArrayList<Map<Integer, String>>();

		List<InvoiceTransSource> modelList = InvoiceTransSource.page();
		if (modelList != null && modelList.size() > 0) {
			for (InvoiceTransSource model : modelList) {
				...
				dataMap.put(i++, model.hasStockEffect.toString());
				dataMap.put(i++, model.hasCostEffect.toString());
		...
		return new PageExtend<InvoiceTransSource>(getHeaderList(), dataList, null);
	}
	...
	public static Result options(String rightName) {
		...
		Result result = ok(StringUtils.buildOptionTag(InvoiceTransSource.options(right), lastSaved));
		lastSaved = null;
	...
	public static Result save() {
		...
		Form<InvoiceTransSource> filledForm = dataForm.bindFromRequest();

		if(filledForm.hasErrors()) {
			return badRequest(form.render(filledForm));
		} else {

			InvoiceTransSource model = filledForm.get();
			...
			if (! model.hasStockEffect) model.hasCostEffect = Boolean.FALSE;
	...
	public static Result create() {
		...
		return ok(form.render(dataForm.fill(new InvoiceTransSource())));
	}
	...
	public static Result edit(Integer id) {
		...
		if (id == null) {
			...
			InvoiceTransSource model = InvoiceTransSource.findById(id);
			if (model == null) {
	 ...
	 * @param filledForm
	 */
	private static void checkConstraints(Form<InvoiceTransSource> filledForm) {
		InvoiceTransSource model = filledForm.get();

		if (InvoiceTransSource.isUsedForElse("name", model.name, model.id)) {
