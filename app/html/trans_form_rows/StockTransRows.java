 ...
 * @author mdpinar
*/
public class StockTransRows {
	
	public static String build(StockTrans stockTrans) {
		StringBuilder row = new StringBuilder();

		List<StockTransDetail> details = stockTrans.details;
		if (details.size() == 0) details.add(new StockTransDetail());
		...
		for (int i = 0; i < details.size(); i++) {
			...
			row.append("<tr>");
				...
				if (! Right.STOK_ACILIS_ISLEMI.equals(stockTrans.right)) {
					row.append("<td>");
					...
					row.append("</td>");
					row.append("<td>");
						row.append("<input type='text' class='currency stocks attention' id='details["+i+"]_discountAmount' name='details["+i+"].discountAmount' value='"+detail.discountAmount+"' style='width:calc(100% - 5px);' />");
					row.append("</td>");
					...
					row.append("</td>");
					if (Profiles.chosen().isFieldVisible(enums.Module.stock, "taxRate2")) {
						row.append("<td>");
					...
					if (Profiles.chosen().isFieldVisible(enums.Module.stock, "taxRate3")) {
						row.append("<td>");
						row.append("<input type='text' class='rate stocks attention' id='details["+i+"]_taxRate3' name='details["+i+"].taxRate3' value='"+detail.taxRate3+"' style='width:calc(100% - 5px);' />");
						row.append("</td>");
					}
					row.append("<td>");
					...
					if (Profiles.chosen().isFieldVisible(enums.Module.stock, "serialNo")) {
						row.append("<td>");
