 ...
 * @author mdpinar
*/
public class WaybillTransRows {

	public static String build(WaybillTrans waybillTrans) {
		StringBuilder row = new StringBuilder();

		List<WaybillTransDetail> details = waybillTrans.details;
		if (details.size() == 0) details.add(new WaybillTransDetail());
		...
		for (int i = 0; i < details.size(); i++) {
			...
			row.append("<tr>");
				...
				row.append("<td>");
					...
					row.append("<input type='hidden' id='details["+i+"]_isTransfer' name='details["+i+"].isTransfer' value='"+detail.isTransfer+"' />");
					if (! Profiles.chosen().irsl_hasPrices) {
						row.append("<input type='hidden' id='details["+i+"]_price' name='details["+i+"].price' value='"+detail.price+"' />");
						row.append("<input type='hidden' id='details["+i+"]_taxRate' name='details["+i+"].taxRate' value='"+detail.taxRate+"' />");
